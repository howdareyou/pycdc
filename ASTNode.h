...
public:
  enum UnOp { UN_POSITIVE, UN_NEGATIVE, UN_INVERT, UN_NOT };
...
public:
  ...
  ASTBinary(PycRef<ASTNode> left, PycRef<ASTNode> right, int op,
      ...
      : ASTNode(type), m_op(op), m_left(std::move(left)),
...
public:
  enum SliceOp { SLICE0, SLICE1, SLICE2, SLICE3 };
...
public:
  enum RetType { RETURN, YIELD, YIELD_FROM };
...
public:
  ...
  ASTCall(PycRef<ASTNode> func, pparam_t pparams, kwparam_t kwparams)
      : ASTNode(NODE_CALL), m_func(std::move(func)),
        m_pparams(std::move(pparams)), m_kwparams(std::move(kwparams)) {}
...
public:
  ...
  ASTImport(PycRef<ASTNode> name, PycRef<ASTNode> fromlist)
      : ASTNode(NODE_IMPORT), m_name(std::move(name)),
        m_fromlist(std::move(fromlist)) {}

  PycRef<ASTNode> name() const { return m_name; }
  list_t stores() const { return m_stores; }
  void add_store(PycRef<ASTStore> store) {
...
public:
  ...
  ASTList(value_t values) : ASTNode(NODE_LIST), m_values(std::move(values)) {}
...
public:
  ...
  ASTSet(value_t values) : ASTNode(NODE_SET), m_values(std::move(values)) {}
...
public:
  enum Word { KW_PASS, KW_BREAK, KW_CONTINUE };
...
public:
  enum InitCond { UNINITED, POPPED, PRE_POPPED };
...
public:
  ASTIterBlock(ASTBlock::BlkType blktype, int start, int end,
               PycRef<ASTNode> iter)
  ...
  void setIndex(PycRef<ASTNode> idx) {
    m_idx = std::move(idx);
    init();
  }
...
public:
  ASTWithBlock(int end) : ASTBlock(ASTBlock::BLK_WITH, end) {}
...
public:
  ...
  ASTFormattedValue(PycRef<ASTNode> val, ConversionFlag conversion,
      ...
      : ASTNode(NODE_FORMATTEDVALUE), m_val(std::move(val)),
...
public:
  ASTAnnotatedVar(PycRef<ASTNode> name, PycRef<ASTNode> type)
      : ASTNode(NODE_ANNOTATED_VAR), m_name(std::move(name)),
