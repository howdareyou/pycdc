...
template <class _Obj> class PycRef {
public:
  ...
  template <class _Cast> PycRef<_Cast> try_cast() const {
    return dynamic_cast<_Cast *>(m_obj);
  }

  template <class _Cast> PycRef<_Cast> cast() const {
...
template <class _Obj> int PycRef<_Obj>::type() const {
