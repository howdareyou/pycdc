...
def decompyle_one(test_name, pyc_file, outdir, tokenized_expect):
    ...
    if tokenized != tokenized_expect:
        ...
        diff = difflib.unified_diff(
            tokenized_expect.splitlines(True),
            tokenized.splitlines(True),
            fromfile=fromfile,
            tofile=tofile,
        )
        diff = list(diff)
...
def run_test(test_file):
    ...
    compiled_files = glob.glob(
        os.path.join(TEST_DIR, "compiled", test_name + ".?.*.pyc")
    )
    ...
    with open(
        os.path.join(TEST_DIR, "tokenized", test_name + ".txt"),
        "r",
        encoding="utf-8",
        errors="replace",
    ) as tok_file:
    ...
    if fails == 0:
        ...
        if xfails != 0:
            status_line += (
                "\033[31mFAIL ({} of {})\033[33m + XFAIL ({})\033[0m\n".format(
                    fails, len(compiled_files), xfails
                )
            )
        else:
            status_line += "\033[31mFAIL ({} of {})\033[0m\n".format(
                fails, len(compiled_files)
            )
...
def main():
    ...
    default_jobs = (
        int(os.environ["JOBS"]) if "JOBS" in os.environ else multiprocessing.cpu_count()
    )
