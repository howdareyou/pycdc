...
typedef std::stack<PycRef<ASTBlock>> ASTBlockStack;

// This must be a triple quote (''' or """), to handle interpolated string
// literals containing the opposite quote style. E.g. f'''{"interpolated "123'
// literal"}'''    -> valid. E.g. f"""{"interpolated "123' literal"}"""    ->
// valid. E.g. f'{"interpolated "123' literal"}'        -> invalid, unescaped
// quotes in literal. E.g. f'{"interpolated \"123\' literal"}'      -> invalid,
// f-string expression does not allow backslash. NOTE: Nested f-strings not
// supported.
#define F_STRING_QUOTE "'''"

static void append_to_chain_store(const PycRef<ASTNode> &chainStore,
                                  PycRef<ASTNode> item, FastStack &stack,
                                  const PycRef<ASTBlock> &curblock);
...
/* Use this to keep track of whether we need to print a class or module
 * docstring */
...
/* compiler generates very, VERY similar byte code for if/else statement block
 * and if-expression statement if a: b = 1 else: b = 2 expression: b = 1 if a
 * else 2 (see for instance https://stackoverflow.com/a/52202007) here, try to
 * guess if just finished else statement is part of if-expression (ternary
 * operator) if it is, remove statements from the block and put a ternary node
 * on top of stack
 */
static void CheckIfExpr(FastStack &stack, PycRef<ASTBlock> curblock) {
  ...
  curblock->removeLast();
  stack.push(new ASTTernary(std::move(if_block), std::move(if_expr),
                            std::move(else_expr)));
}

void ParseSource(PycBuffer &source, PycRef<PycCode> code, PycModule *mod,
                 ASTBlockStack &blocks) {
  FastStack stack((mod->majorVer() == 1) ? 20 : code->stackSize());
  stackhist_t stack_hist;

  PycRef<ASTBlock> defblock = blocks.top();
  ...
  while (!source.atEof()) {
    ...
    if (need_try && opcode != Pyc::SETUP_EXCEPT_A) {
      ...
      PycRef<ASTBlock> tryblock =
          new ASTBlock(ASTBlock::BLK_TRY, curblock->end(), true);
      blocks.push(tryblock);
      curblock = blocks.top();
    } else if (else_pop && curblock->inited() == ASTCondBlock::POPPED) {
      PycRef<ASTNode> cond = stack.top();
      stack.pop();

      stack_hist.pop();

      if (curblock.cast<ASTCondBlock>()->negative()) {
        PycRef<ASTBinary> newcond = new ASTBinary(
            curblock.cast<ASTCondBlock>()->cond(), cond, ASTBinary::BIN_LOG_OR);
        stack.push(newcond.cast<ASTNode>());
      } else {
        PycRef<ASTBinary> newcond =
            new ASTBinary(curblock.cast<ASTCondBlock>()->cond(), cond,
    ...
    case Pyc::INPLACE_MATRIX_MULTIPLY: {
      ...
      if (op == ASTBinary::BIN_INVALID)
        throw std::runtime_error(
            "Unhandled opcode from ASTBinary::from_opcode");
      PycRef<ASTNode> right = stack.top();
      stack.pop();
      PycRef<ASTNode> left = stack.top();
      stack.pop();
    ...
    case Pyc::KW_NAMES_A: {
      ...
      std::vector<PycRef<PycObject>> keys =
          code->getConst(operand).cast<PycSimpleSequence>()->values();
    ...
    case Pyc::INSTRUMENTED_CALL_A: {
      ...
      /*
      KW_NAMES(i)
          Stores a reference to co_consts[consti] into an internal variable for
      use by CALL. co_consts[consti] must be a tuple of strings. New in
      version 3.11.
      */
      if (mod->verCompare(3, 11) >= 0) {
        ...
        if (object_or_map.type() == ASTNode::NODE_KW_NAMES_MAP) {
          stack.pop();
          PycRef<ASTKwNamesMap> kwparams_map =
              object_or_map.cast<ASTKwNamesMap>();
          for (ASTKwNamesMap::map_t::const_iterator it =
                   kwparams_map->values().begin();
      ...
      for (int i = 0; i < pparams; i++) {
        ...
        if (param.type() == ASTNode::NODE_FUNCTION) {
          ...
          PycRef<PycCode> code_src =
              fun_code.cast<ASTObject>()->object().cast<PycCode>();
    ...
    case Pyc::CALL_FUNCTION_KW_A: {
      ...
      stack.pop();
      PycSimpleSequence::value_t kwparam_keys =
          kw.cast<ASTObject>()->object().cast<PycTuple>()->values();
      auto first_kwparam_key = kwparam_keys.rbegin();

      size_t kwparams = kwparam_keys.size();
      size_t pparams = operand - kwparams;
      ASTCall::kwparam_t kwparamList;
      ASTCall::pparam_t pparamList;
      for (size_t i = 0; i < kwparams; i++) {
        PycRef<ASTNode> val = stack.top();
        stack.pop();
        PycRef<ASTObject> key = new ASTObject(*(first_kwparam_key + i));
        kwparamList.push_front(std::make_pair(key.cast<ASTNode>(), val));
      }
      for (size_t i = 0; i < pparams; i++) {
        pparamList.push_front(stack.top());
        stack.pop();
      }
      PycRef<ASTNode> func = stack.top();
    ...
    case Pyc::CONTAINS_OP_A: {
      ...
      stack.push(new ASTCompare(
          left, right, operand ? ASTCompare::CMP_NOT_IN : ASTCompare::CMP_IN));
    } break;
    case Pyc::DELETE_ATTR_A: {
      PycRef<ASTNode> name = stack.top();
      stack.pop();
      curblock->append(new ASTDelete(new ASTBinary(
          name, new ASTName(code->getName(operand)), ASTBinary::BIN_ATTR)));
    } break;
    ...
    case Pyc::DELETE_SLICE_0: {
      ...
      curblock->append(
          new ASTDelete(new ASTSubscr(name, new ASTSlice(ASTSlice::SLICE0))));
    } break;
    case Pyc::DELETE_SLICE_1: {
      ...
      curblock->append(new ASTDelete(
          new ASTSubscr(name, new ASTSlice(ASTSlice::SLICE1, upper))));
    } break;
    case Pyc::DELETE_SLICE_2: {
      ...
      curblock->append(new ASTDelete(
          new ASTSubscr(name, new ASTSlice(ASTSlice::SLICE2, NULL, lower))));
    } break;
    case Pyc::DELETE_SLICE_3: {
      ...
      curblock->append(new ASTDelete(
          new ASTSubscr(name, new ASTSlice(ASTSlice::SLICE3, upper, lower))));
    } break;
    ...
    case Pyc::END_FINALLY: {
      ...
      if (curblock->blktype() == ASTBlock::BLK_FINALLY) {
        ...
        if (blocks.top()->blktype() == ASTBlock::BLK_CONTAINER) {
          ...
          isUninitAsyncFor =
              asyncForBlock->blktype() == ASTBlock::BLK_ASYNCFOR &&
              !asyncForBlock->inited();
          if (isUninitAsyncFor) {
            auto tryBlock = container->nodes().front().cast<ASTBlock>();
            if (!tryBlock->nodes().empty() &&
                tryBlock->blktype() == ASTBlock::BLK_TRY) {
        ...
        if (!isUninitAsyncFor) {
          ...
          if (curblock->end() != pos ||
            ...
            PycRef<ASTBlock> elseblk =
                new ASTBlock(ASTBlock::BLK_ELSE, prev->end());
            elseblk->init();
            blocks.push(elseblk);
            curblock = blocks.top();
          } else {
    ...
    case Pyc::INSTRUMENTED_FOR_ITER_A: {
      ...
      // before 3.8, there is a SETUP_LOOP instruction with block start and end
      // position,
      //    the operand is usually a jump to a POP_BLOCK instruction
      // after 3.8, block extent has to be inferred implicitly; the operand is a
      // jump to a position after the for block
      ...
      PycRef<ASTIterBlock> forblk =
          new ASTIterBlock(ASTBlock::BLK_FOR, curpos, end, iter);
    ...
    case Pyc::FOR_LOOP_A: {
      ...
      PycRef<ASTIterBlock> forblk =
          new ASTIterBlock(ASTBlock::BLK_FOR, curpos, top->end(), iter);
      forblk->setComprehension(comprehension);
      blocks.push(forblk.cast<ASTBlock>());
      curblock = blocks.top();
    ...
    case Pyc::GET_AITER: {
      ...
      if (top->blktype() == ASTBlock::BLK_WHILE) {
        blocks.pop();
        PycRef<ASTIterBlock> forblk =
            new ASTIterBlock(ASTBlock::BLK_ASYNCFOR, curpos, top->end(), iter);
        blocks.push(forblk.cast<ASTBlock>());
        curblock = blocks.top();
        stack.push(nullptr);
      } else {
    ...
    case Pyc::FORMAT_VALUE_A: {
      auto conversion_flag =
          static_cast<ASTFormattedValue::ConversionFlag>(operand);
    ...
    case Pyc::IMPORT_NAME_A:
      if (mod->majorVer() == 1) {
        ...
        stack.push(
            new ASTImport(new ASTName(code->getName(operand)), fromlist));
      }
      break;
    ...
    case Pyc::IS_OP_A: {
      ...
      stack.push(new ASTCompare(
          left, right, operand ? ASTCompare::CMP_IS_NOT : ASTCompare::CMP_IS));
    } break;
    ...
    case Pyc::INSTRUMENTED_POP_JUMP_IF_TRUE_A: {
      ...
      if (mod->verCompare(3, 12) >= 0 || opcode == Pyc::JUMP_IF_FALSE_A ||
          opcode == Pyc::JUMP_IF_TRUE_A ||
          opcode == Pyc::POP_JUMP_FORWARD_IF_TRUE_A ||
          opcode == Pyc::POP_JUMP_FORWARD_IF_FALSE_A) {
      ...
      if (cond.type() == ASTNode::NODE_COMPARE &&
        ...
        ifblk = new ASTCondBlock(ASTBlock::BLK_EXCEPT, offs,
        ...
        ifblk = new ASTCondBlock(ASTBlock::BLK_ELIF, offs, cond, neg);
                 ...
                 offs == curblock->end() - 2) {
        /* If the condition jumps to the loop exit,
           it is the condition for the loop */
        PycRef<ASTBlock> top = blocks.top();
        blocks.pop();
        ...
        int body_end = offs;
        if (offs < pos && curblock->blktype() == ASTBlock::BLK_WHILE)
          body_end = curblock->end() - 4;
        /* Plain old if statement */
        ifblk = new ASTCondBlock(ASTBlock::BLK_IF, body_end, cond, neg);
      }
    ...
    case Pyc::JUMP_ABSOLUTE_A: {
      ...
      if (offs < pos) {
        if (curblock->blktype() == ASTBlock::BLK_FOR) {
          bool is_jump_to_start =
              offs == curblock.cast<ASTIterBlock>()->start();
          bool should_pop_for_block =
              curblock.cast<ASTIterBlock>()->isComprehension();
          // in v3.8, SETUP_LOOP is deprecated and for blocks aren't terminated
          // by POP_BLOCK, so we add them here
          bool should_add_for_block =
              mod->majorVer() == 3 && mod->minorVer() >= 8 &&
              is_jump_to_start &&
              !curblock.cast<ASTIterBlock>()->isComprehension();
      ...
      if (curblock->blktype() == ASTBlock::BLK_CONTAINER) {
        ...
        if (cont->hasExcept() && pos < cont->except()) {
          PycRef<ASTBlock> except =
              new ASTCondBlock(ASTBlock::BLK_EXCEPT, 0, NULL, false);
          except->init();
          blocks.push(except);
          curblock = blocks.top();
        }
      ...
      do {
        ...
        if (prev->blktype() == ASTBlock::BLK_IF ||
          ...
          PycRef<ASTBlock> next =
              new ASTBlock(ASTBlock::BLK_ELSE, blocks.top()->end());
          ...
          PycRef<ASTBlock> next = new ASTCondBlock(
              ASTBlock::BLK_EXCEPT, blocks.top()->end(), NULL, false);
          next->init();
    ...
    case Pyc::INSTRUMENTED_JUMP_FORWARD_A: {
      ...
      if (curblock->blktype() == ASTBlock::BLK_CONTAINER) {
        ...
        if (cont->hasExcept()) {
          ...
          PycRef<ASTBlock> except =
              new ASTCondBlock(ASTBlock::BLK_EXCEPT, pos + offs, NULL, false);
          except->init();
          blocks.push(except);
          curblock = blocks.top();
        }
      ...
      if (curblock->blktype() == ASTBlock::BLK_IF &&
          curblock->inited() == ASTCondBlock::POPPED)
        break;

      if (!stack_hist.empty()) {
        if (stack.empty()) // if it's part of if-expression, TOS at the moment
                           // is the result of "if" part
          stack = stack_hist.top();
        stack_hist.pop();
      }
      ...
      do {
        ...
        if ((prev->blktype() == ASTBlock::BLK_IF) ||
          ...
          if (blocks.top()->blktype() == ASTBlock::BLK_CONTAINER) {
            PycRef<ASTContainerBlock> cont =
                blocks.top().cast<ASTContainerBlock>();
            if (cont->hasExcept()) {
              if (push) {
                stack_hist.push(stack);
              }

              PycRef<ASTBlock> except = new ASTCondBlock(
                  ASTBlock::BLK_EXCEPT, pos + offs, NULL, false);
              except->init();
              blocks.push(except);
            }
          } else {
      ...
      if (name.type() != ASTNode::NODE_IMPORT) {
        ...
        if (mod->verCompare(3, 12) >= 0) {
          if (operand & 1) {
            ...
            If the low bit of name is set, then a NULL or self is pushed to the
            stack before the attribute or unbound method respectively. */
            stack.push(nullptr);
          }
          operand >>= 1;
        }

        stack.push(new ASTBinary(name, new ASTName(code->getName(operand)),
                                 ASTBinary::BIN_ATTR));
      }
    } break;
    ...
    case Pyc::POP_BLOCK: {
      ...
      if (curblock->blktype() == ASTBlock::BLK_IF ||
        ...
        if (!stack_hist.empty()) {
          ...
          fprintf(stderr, "Warning: Stack history is empty, something wrong "
                          "might have happened\n");
        }
      }
      PycRef<ASTBlock> tmp = curblock;
      ...
      if (!(tmp->blktype() == ASTBlock::BLK_ELSE && tmp->nodes().size() == 0)) {
        curblock->append(tmp.cast<ASTNode>());
      }
      ...
      if (curblock->blktype() == ASTBlock::BLK_CONTAINER) {
        ...
        } else if ((tmp->blktype() == ASTBlock::BLK_ELSE &&
                    cont->hasFinally()) ||
    ...
    case Pyc::PRINT_ITEM: {
      ...
      if (curblock->size() > 0 &&
          curblock->nodes().back().type() == ASTNode::NODE_PRINT)
    ...
    case Pyc::RAISE_VARARGS_A: {
      ...
      if ((curblock->blktype() == ASTBlock::BLK_IF ||
           curblock->blktype() == ASTBlock::BLK_ELSE) &&
          stack_hist.size() && (mod->verCompare(2, 6) >= 0)) {
        stack = stack_hist.top();
        stack_hist.pop();
    ...
    case Pyc::WITH_CLEANUP_START: {
      ...
      if (curblock->blktype() == ASTBlock::BLK_WITH &&
        ...
        fprintf(stderr,
                "Something TERRIBLE happened! No matching with block found for "
                "WITH_CLEANUP at %d\n",
                curpos);
      }
    } break;
    ...
    case Pyc::SETUP_EXCEPT_A: {
      ...
      stack_hist.push(stack);
      PycRef<ASTBlock> tryblock =
          new ASTBlock(ASTBlock::BLK_TRY, pos + operand, true);
    ...
    case Pyc::SETUP_LOOP_A: {
      PycRef<ASTBlock> next =
          new ASTCondBlock(ASTBlock::BLK_WHILE, pos + operand, NULL, false);
      next->init();
      blocks.push(next.cast<ASTBlock>());
      curblock = blocks.top();
    } break;
    ...
    case Pyc::STORE_ATTR_A: {
      if (unpack) {
        PycRef<ASTNode> name = stack.top();
        stack.pop();
        PycRef<ASTNode> attr = new ASTBinary(
            name, new ASTName(code->getName(operand)), ASTBinary::BIN_ATTR);
    ...
    case Pyc::STORE_FAST_A: {
      if (unpack) {
        ...
        if (--unpack <= 0) {
          ...
          if (curblock->blktype() == ASTBlock::BLK_FOR && !curblock->inited()) {
    ...
    case Pyc::STORE_SLICE_0: {
      ...
      curblock->append(new ASTStore(
          value, new ASTSubscr(dest, new ASTSlice(ASTSlice::SLICE0))));
    } break;
    case Pyc::STORE_SLICE_1: {
      ...
      curblock->append(new ASTStore(
          value, new ASTSubscr(dest, new ASTSlice(ASTSlice::SLICE1, upper))));
    } break;
    case Pyc::STORE_SLICE_2: {
      ...
      curblock->append(new ASTStore(
          value,
          new ASTSubscr(dest, new ASTSlice(ASTSlice::SLICE2, NULL, lower))));
    } break;
    case Pyc::STORE_SLICE_3: {
      ...
      curblock->append(new ASTStore(
          value,
          new ASTSubscr(dest, new ASTSlice(ASTSlice::SLICE3, upper, lower))));
    } break;
    case Pyc::STORE_SUBSCR: {
      if (unpack) {
        ...
        // If variable annotations are enabled, we'll need to check for them
        // here. Python handles a varaible annotation by setting:
        // __annotations__['var-name'] = type
        const bool found_annotated_var =
            (variable_annotations && dest->type() == ASTNode::Type::NODE_NAME &&
        ...
        if (found_annotated_var) {
          ...
          if (!curblock->nodes().empty() &&
            ...
            // Replace the existing NODE_STORE with a new one that includes the
            // annotation.
            PycRef<ASTStore> store = curblock->nodes().back().cast<ASTStore>();
            curblock->removeLast();
            curblock->append(
                new ASTStore(store->src(), new ASTAnnotatedVar(subscr, src)));
          } else {
          ...
          if (dest.type() == ASTNode::NODE_MAP) {
            ...
            append_to_chain_store(src, new ASTSubscr(dest, subscr), stack,
                                  curblock);
          } else {
    ...
    case Pyc::UNPACK_SEQUENCE_A: {
      ...
      if (unpack > 0) {
        ...
        // Unpack zero values and assign it to top of stack or for loop
        // variable. E.g. [] = TOS / for [] in X
        ASTTuple::value_t vals;
...
PycRef<ASTNode> BuildFromCode(PycRef<PycCode> code, PycModule *mod) {
  ...
  ParseSource(source, code, mod, blocks);
...
static int cmp_prec(PycRef<ASTNode> parent, PycRef<ASTNode> child) {
  ...
  if (parent.type() == ASTNode::NODE_UNARY &&
      parent.cast<ASTUnary>()->op() == ASTUnary::UN_NOT)
  ...
  if (child.type() == ASTNode::NODE_BINARY) {
    ...
    if (parent.type() == ASTNode::NODE_BINARY) {
      ...
      return (binChild->op() == ASTBinary::BIN_LOG_AND ||
              binChild->op() == ASTBinary::BIN_LOG_OR)
                 ? 1
                 : -1;
    ...
    if (parent.type() == ASTNode::NODE_BINARY)
      return (parent.cast<ASTBinary>()->op() == ASTBinary::BIN_LOG_AND ||
              parent.cast<ASTBinary>()->op() == ASTBinary::BIN_LOG_OR)
                 ? -1
                 : 1;
    else if (parent.type() == ASTNode::NODE_COMPARE)
...
void print_src(PycRef<ASTNode> node, PycModule *mod, std::ostream &pyc_output) {
  ...
  case ASTNode::NODE_CALL: {
    ...
    for (const auto &param : call->kwparams()) {
      ...
      if (param.first.type() == ASTNode::NODE_NAME) {
        ...
        PycRef<PycString> str_name =
            param.first.cast<ASTObject>()->object().cast<PycString>();
  ...
  case ASTNode::NODE_EXEC: {
    ...
    if (exec->globals() != NULL) {
      ...
      if (exec->locals() != NULL && exec->globals() != exec->locals()) {
        pyc_output << ", ";
  ...
  case ASTNode::NODE_JOINEDSTR:
    ...
    for (const auto &val : node.cast<ASTJoinedStr>()->values()) {
      ...
      case ASTNode::NODE_OBJECT:
        // When printing a piece of the f-string, keep the quote style
        // consistent. This avoids problems when ''' or """ is part of the
        // string.
        print_const(pyc_output, val.cast<ASTObject>()->object(), mod,
                    F_STRING_QUOTE);
        break;
      default:
  ...
  case ASTNode::NODE_CONST_MAP: {
    ...
    PycTuple::value_t keys = const_map->keys()
                                 .cast<ASTObject>()
                                 ->object()
                                 .cast<PycTuple>()
                                 ->values();
  ...
  case ASTNode::NODE_BLOCK: {
    ...
    if (blk->blktype() == ASTBlock::BLK_IF ||
      ...
      PycRef<ASTCondBlock> cond_block = blk.cast<ASTCondBlock>();
      if (blk->blktype() == ASTBlock::BLK_WHILE &&
          cond_block->cond() == nullptr) {
        /* If the condition is None, we need to set it to True */
        PycRef<ASTObject> pyTrue = new ASTObject(Pyc_True);
        cond_block->setCond(pyTrue.cast<ASTNode>());
      }

      print_src(cond_block->cond(), mod, pyc_output);
  ...
  case ASTNode::NODE_RETURN: {
    ...
    if (value.type() == ASTNode::NODE_TUPLE)
      value.cast<ASTTuple>()->setRequireParens(false);
  ...
  case ASTNode::NODE_IMPORT: {
    ...
    if (import->stores().size()) {
      ...
      if (stores.size() == 1) {
        ...
        if (src.cast<ASTName>()->name()->value() !=
            dest.cast<ASTName>()->name()->value()) {
          pyc_output << " as ";
        ...
        for (const auto &st : stores) {
          ...
          if (st->src().cast<ASTName>()->name()->value() !=
              st->dest().cast<ASTName>()->name()->value()) {
            pyc_output << " as ";
  ...
  case ASTNode::NODE_STORE: {
    ...
    if (src.type() == ASTNode::NODE_FUNCTION) {
      ...
      PycRef<ASTNode> code = src.cast<ASTClass>()
                                 ->code()
                                 .cast<ASTCall>()
                                 ->func()
                                 .cast<ASTFunction>()
      ...
      if (import->fromlist() != NULL) {
        PycRef<PycObject> fromlist =
            import->fromlist().cast<ASTObject>()->object();
        ...
        if (!dest.cast<ASTName>()->name()->isEqual(
                import_name.cast<ASTName>()->name().cast<PycObject>())) {
          pyc_output << " as ";
          print_src(dest, mod, pyc_output);
        }
      }
      ...
      if (dest.type() == ASTNode::NODE_TUPLE)
        dest.cast<ASTTuple>()->setRequireParens(false);
      print_src(dest, mod, pyc_output);
      pyc_output << " = ";
      print_src(src, mod, pyc_output);
    }
  ...
  case ASTNode::NODE_TERNARY: {
     ...
     *
     * when if-expr is part of numerical expression, ternary has the LOWEST
     * precedence print(a + b if False else c) output is c, not a+c (a+b is
     * calculated first)
     *
     * but, let's not add parenthesis - to keep the source as close to original
...
bool print_docstring(PycRef<PycObject> obj, int indent, PycModule *mod,
                     std::ostream &pyc_output) {
  // docstrings are translated from the bytecode __doc__ = 'string' to simply
  // '''string'''
...
void decompyle(PycRef<PycCode> code, PycModule *mod, std::ostream &pyc_output) {
  ...
  if (cleanBuild) {
    ...
    // Class and module docstrings may only appear at the beginning of their
    // source
